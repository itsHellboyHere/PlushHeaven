...
const CartItem = ({cartItem}) => {
    const {cartID,name,image,price ,amount,productColor ,company} =cartItem;
  ...
  return (
    <article key={cartID} className="mb-12 flex flex-col gap-y-4 sm:flex-row flex-wrap border-b border-base-300 pb-6 last:border-b-0">
        {/* IMAGE */}
        <img src={image} alt={name} className="h-24 w-24 rounded-lg sm:h-32 sm:w-32 object-cover"/>
        {/* INFO */}
        <div className="sm:ml-16 sm:w-48">
            {/* TITLE */}
            <h3 className="capitalize font-medium">{name}</h3>
            {/* COMPANY */}
             <h3 className="mt-2 capitalize  text-sm text-neutral-content">{company}</h3>
            {/* COLOR */}
